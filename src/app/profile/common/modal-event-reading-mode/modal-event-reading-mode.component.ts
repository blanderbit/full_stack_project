...
export class ModalEventReadingModeComponent implements OnInit, DoCheck, OnDestroy {
    ...
    tabClick(event: MouseEvent) {
        ...
        if(attr == null || attr.value == null) return;
        ...
        parseInt(tabIndex) === 1 ? this.subscribers(
            this.event_service.getLikeForEvent(this.id, 1),
            'likes',
        ) : null;
        parseInt(tabIndex) === 2 ? this.subscribers(
            this.event_service.getCommentForEvent(this.id, 1),
            'comments',
        ) : null;
    }
    subscribers(observers: any, name: string, more = null) {
          observers.subscribe(next => {
              if (name === 'likes' || name === 'comments') {
                  this.data[name] =  this.data[name].concat(next.data);
                  delete next['data'];
                  this.pagination[name] = next;
              } else {
                  this.data[name] = next;
              }
          });
    ...
    getMore (name) {
        switch (name) {
            case 'likes': this.subscribers(
                this.event_service.getLikeForEvent(
                    this.id,
                    this.pagination[name].current_page + 1 ),
                'likes', 'more');
                break;
            case 'comments': this.subscribers(
                this.event_service.getCommentForEvent(
                    this.id,
                    this.pagination[name].current_page + 1 ),
                'likes', 'more');
                break;
    ...
    timeAgoNaive2(comparisonDate) {
        ...
        for (let i = 0 ; i < 9; i += 1) {
            ...
            if (i === 6 && this.timeparts[i].name === 'hour' &&
                now_day.time === 0 && now_day.day === 5) value -= 2;
            // debugger;
            if (value) {
                // if(parts.length === 1) break;
                parts.push(value + ' ' +
                    this.timeparts[i].name +
                    (value !== 1 ? this.timeparts[i].p || 's' : this.timeparts[i].s || ''));
            }
        }
